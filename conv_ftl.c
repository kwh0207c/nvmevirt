...
{	
	#if (GC_MODE == GC_MODE_GREEDY || GC_MODE == GC_MODE_FIFO)
	return (next > curr);
...
{
    /* SLCB: SLC lines to be prioritized for migration */
    #if (GC_MODE == GC_MODE_GREEDY)
    struct line *l = (struct line *)a;
    uint32_t pri = l->vpc;
	if (!(l->is_slc)) pri |= (1U << 30);
    return pri;
...
static void init_lines(struct conv_ftl *conv_ftl)
{
	...
	/* SLCB: initialize slc line lists */
	INIT_LIST_HEAD(&lm->slc_free_line_list);
	...
	/* SLCB: Calculate SLC line count */
	uint32_t slc_line_count = (lm->tt_lines * SLC_PORTION) / 100;

	for (i = 0; i < lm->tt_lines; i++) {
		bool is_slc = (i < slc_line_count);
		...
		/* SLCB: line partition */
		if (is_slc) {
			list_add_tail(&lm->lines[i].entry, &lm->slc_free_line_list);
...
static struct line *get_next_free_line(struct conv_ftl *conv_ftl, bool is_slc)
{
    ...
    uint32_t *target_cnt = is_slc ? &lm->slc_free_line_cnt : &lm->free_line_cnt;

    struct line *curline = list_first_entry_or_null(target_list, struct line, entry);
...
static void prepare_write_pointer(struct conv_ftl *conv_ftl, uint32_t io_type)
{
    ...
    /* SLCB: USER_IO starts in SLC; GC_IO (Migration/GC) goes to TLC */
    bool is_slc = (io_type == USER_IO); 
    curline = get_next_free_line(conv_ftl, is_slc);

	/* SLCB: Spill-over check; If SLC is empty at start, try TLC for USER_IO */
    if (!curline && is_slc) {
...
static void advance_write_pointer(struct conv_ftl *conv_ftl, uint32_t io_type)
{
	...
	/* SLCB */
	bool was_slc = (wpp->curline->is_slc);

	// Try to get next SLC line if we are doing USER_IO
	if (io_type == USER_IO) {
		wpp->curline = get_next_free_line(conv_ftl, true); // Try SLC
		if (!wpp->curline) {
			wpp->curline = get_next_free_line(conv_ftl, false); // Fallback to TLC
		}
	} 
	else {
		wpp->curline = get_next_free_line(conv_ftl, false); // GC always to TLC
	}
...
static void mark_page_invalid(struct conv_ftl *conv_ftl, struct ppa *ppa)
{
	...
	#if (GC_MODE == GC_MODE_GREEDY)
	if (line->pos) {
		pqueue_change_priority(lm->victim_line_pq, victim_line_get_pri(line), line);
	} 
	#endif
	...
	if (was_full_line) {
		...
		/* SLCB */
		if (line->is_slc) {
			list_add_tail(&line->entry, &lm->slc_victim_line_list);
			lm->slc_victim_line_cnt++;
		}
		else {
			list_add_tail(&line->entry, &lm->victim_line_list);
			lm->victim_line_cnt++;
		}
...
static uint64_t gc_write_page(struct conv_ftl *conv_ftl, struct ppa *old_ppa)
{
	...
	#if (GC_MODE == GC_MODE_CB || GC_MODE == GC_MODE_FIFO)
...
static struct line *select_victim_line(struct conv_ftl *conv_ftl, bool force)
{
	...
	uint32_t *target_line_cnt;
	if (lm->slc_victim_line_cnt > 0) {
		target_line_list = &lm->slc_victim_line_list;
		target_line_cnt = &lm->slc_victim_line_cnt;
	}
	else {
		target_line_list = &lm->victim_line_list;
		target_line_cnt = &lm->victim_line_cnt;
	}
	...
	*/
	...
	/* Iteration */
    list_for_each_entry(line, target_line_list, entry) {
	...
	#elif (GC_MODE == GC_MODE_FIFO)
	victim_line = list_first_entry_or_null(target_line_list, struct line, entry);
...
static void mark_line_free(struct conv_ftl *conv_ftl, struct ppa *ppa)
{
	...
	/* SLCB */
	if (line->is_slc) {
        list_add_tail(&line->entry, &lm->slc_free_line_list);
        lm->slc_free_line_cnt++;
    } else {
...
static bool conv_write(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
		...
		#if (GC_MODE == GC_MODE_CB || GC_MODE == GC_MODE_FIFO)
		/* CBGC: update line timestamp */
		cur_line->last_update = ktime_get();
		#endif
